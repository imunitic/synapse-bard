---
name: synapse-bard-sync
description: Run synapse-bard sync to (re)populate _bard/graph/ from the bible's real source tree — the discoverable entry point for the CLI's own `sync` subcommand, since a user has no reason to already know that binary exists. Use whenever the author wants the Bible-graph refreshed after adding or editing entity files ("sync the bible graph", "refresh _bard/graph", "run bard sync").
---
...
Run `synapse-bard sync` (a shell call — this is the CLI binary, not something this skill
